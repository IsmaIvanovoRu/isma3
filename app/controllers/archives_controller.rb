class ArchivesController < ApplicationController
  ...
  def index
    if current_user.nil?
      @articles = Article.order('updated_at DESC').where(published: true, group_id: nil).where(article_params).paginate(:page => params[:page])
    else
      if current_user_moderator?
	current_user_groups = Group.all + [nil]
	@articles = Article.order('updated_at DESC').where(group_id: current_user_groups).where(article_params).paginate(:page => params[:page])
      else
	current_user_groups = current_user.groups + current_user.groups.joins(:parent).map{|g| g.parent} + [nil]
	@articles = Article.order('updated_at DESC').where(group_id: current_user_groups, published: true).where(article_params).paginate(:page => params[:page])
      end
    end
  end
